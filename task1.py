...
class BloomFilter:
    ...
    def add(self, item):
        if not isinstance(item, str) or not item:
            raise ValueError(
                "Елемент має бути наявним та не може бути порожнім рядком.")
...
def check_password_uniqueness(bloom_filter, passwords):
    ...
    for password in passwords:
        if not isinstance(password, str) or not password:
            results[password] = "пароль відсутній чи не є рядком"
            continue
...
if __name__ == "__main__":
    ...
    for password, status in results.items():
     ...
     # Перевірка паролів, які не є дозволенними
    new_passwords_to_check = [123, ""]
    results = check_password_uniqueness(bloom, new_passwords_to_check)

    for password, status in results.items():
        print(f"Пароль '{password}' - {status}.")

    new_passwords_to_check = "password"
    results = check_password_uniqueness(bloom, new_passwords_to_check)
